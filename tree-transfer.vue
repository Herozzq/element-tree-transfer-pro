<template>
  <div class="wl-transfer transfer" :style="{ width, height }">
    ...
    <div class="transfer-center right-list-center">
      <p class="transfer-center-item right-only-item">
        <el-button
          type="primary"
          @click="rightListTransfer()"
          icon="el-icon-arrow-right"
          class="right-first-btn"
        ...
        <div class="transfer-main">
          ...
          <ul class="right-list-ul">
            ...
            >
              ...
              <i
                class="right-list-del el-icon-close"
...
export default {
  ...
  watch: {
    ...
    filterListFirst(newval, oldval) {
      if (oldval == "") {
        this.archiveFirst = this.rightee;
      }
      if (newval == "") {
        this.rightee = this.archiveFirst;
      }
      let reg = RegExp(newval);
      this.rightee = this.rightee.filter((item) => reg.test(item.label));
    },
  ...
  methods: {
    ...
    rightListTransfer() {
      ...
      this.rightee = [...this.rightee, ...arrayDeWeighting];
      console.log("穿梭", this.rightee);

      if (this.isRadio) {
        this.setDisable(this.self_from_data);
      } else {
        for (const item of this.rightee) {
          item.disabled = true;
        }
      }
      // 传递信息给父组件
      this.$emit("add-btn", this.rightee);
    },
    // 清理 右侧选中 数据
    clearList(id) {
      this.rightee =
...
.right-list-center > .transfer-center-item {
  height: 50%;
  padding: 70px 10px 0;
  box-sizing: border-box;
  overflow: hidden;
...
.right-list-center > .right-only-item {
  height: 100%;
  position: relative;
}

.right-only-item > .right-first-btn {
  position: absolute;
  top: 50%;
  left: 50%;
...
.right-list-li:hover .right-list-del {
  display: block;
}
